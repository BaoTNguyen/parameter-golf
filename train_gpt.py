# =============================================================================
# train_h100.py — H100 submission script
# Based on train_gpt_v2.py with all locally-validated improvements baked in.
#
# Validated changes vs SOTA record (PR #1019):
#   1. MODEL_DIM=576 (+27% params, −0.12 pre-quant BPB locally)
#   2. INT5 MLP QAT (MLP_QAT_ENABLED, MLP_QAT_CLIP=15)  — −0.113 post-quant BPB
#   3. INT5 MLP GPTQ export (GPTQ_MLP_CLIP=15)           — saves ~1.1 MB artifact
#   4. Early MLP QAT threshold 0.30 (vs 0.15)            — −0.114 pre-quant BPB
#   5. 32-seq AR calibration (GPTQ_CALIB_SEQS=32)        — saves ~90s on H100
#   6. BIGRAM_VOCAB_SIZE=3072, BIGRAM_DIM=112             — from SOTA record
#
# Run on 8×H100:
#   torchrun --standalone --nproc_per_node=8 train_h100.py
#
# All values remain overridable via env vars for ablations.
# =============================================================================
...
# [FA3 fallback] Use Flash Attention 3 (Hopper) if available, else PyTorch SDPA
try:
    from flash_attn_interface import flash_attn_func as flash_attn_3_func
    _HAS_FA3 = True
except ImportError:
    _HAS_FA3 = False
    flash_attn_3_func = None  # type: ignore
class Hyperparameters:
    ...
    model_dim = int(os.environ.get("MODEL_DIM", 576))   # wider: +27% params, fits ~9.1 MB at INT5
    ...
    bigram_vocab_size = int(os.environ.get("BIGRAM_VOCAB_SIZE", 3072))  # from SOTA record
    bigram_dim = int(os.environ.get("BIGRAM_DIM", 112))                 # from SOTA record
    ...
    # [Insight A] 32-seq calibration saves ~90s on H100 (validated: −0.028 BPB, acceptable tradeoff)
    gptq_calib_seqs = int(os.environ.get("GPTQ_CALIB_SEQS", 32))
    # [Insight C] INT5 MLP GPTQ export — saves ~1.1 MB vs INT6, frees space for wider model
    gptq_mlp_clip = int(os.environ.get("GPTQ_MLP_CLIP", 15))  # 15=int5, 31=int6, 7=int4
    # [Insight C] INT5 MLP QAT — STE on bank weights during warmdown (validated: −0.113 post-quant BPB)
    mlp_qat_enabled = bool(int(os.environ.get("MLP_QAT_ENABLED", "1")))
    mlp_qat_clip = int(os.environ.get("MLP_QAT_CLIP", 15))  # 15=int5(default), 7=int4
    # Early MLP QAT threshold — 0.30 activates ~step 2000/3000 on H100 (validated: −0.114 pre-quant BPB)
    mlp_qat_threshold = float(os.environ.get("MLP_QAT_THRESHOLD", 0.30))
    # [Insight B] Entropy regularization weight during warmdown (~200 bytes)
    entropy_reg_weight = float(os.environ.get("ENTROPY_REG_WEIGHT", 0.0))  # 0=disabled
...
# [FA3 fallback] Unified attention dispatch for Hopper (FA3) and non-Hopper (SDPA)
def _causal_attn(q: Tensor, k: Tensor, v: Tensor, num_heads: int, num_kv_heads: int) -> Tensor:
    """q: (B,T,H,D), k: (B,T,Hkv,D), v: (B,T,Hkv,D) -> (B,T,H,D)"""
    if _HAS_FA3:
    ...
    y = F.scaled_dot_product_attention(q_s, k_s, v_s, is_causal=True, scale=1.0)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        y = _causal_attn(q, k, v, self.num_heads, self.num_kv_heads)
        if self.use_xsa:
...
# [Insight C] STE for MLP bank weights — clips to INT4/INT5 grid in forward, passes gradient through
_MLP_QAT_ACTIVE = False  # toggled in training loop during warmdown
_MLP_QAT_CLIP = 15       # set from args.mlp_qat_clip (15=int5, 7=int4)

def _ste_quant_bank(w: Tensor, clip_range: int) -> Tensor:
    """Straight-Through Estimator: round to int grid in forward, pass gradient through."""
    with torch.no_grad():
        w32 = w.float()
        row_max = w32.abs().amax(dim=-1)
        scale = (row_max / clip_range).clamp_min(1.0 / clip_range)
        # Reshape scale for broadcast: bank weights are 3D (layers, out, in) or 2D
        for _ in range(w32.ndim - scale.ndim):
            scale = scale.unsqueeze(-1)
        w_q = (torch.clamp(torch.round(w32 / scale), -clip_range, clip_range) * scale).to(w.dtype)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        # [Insight C] Apply STE quantization to MLP banks during warmdown if enabled
        if _MLP_QAT_ACTIVE and self.training:
            up_w = _ste_quant_bank(up_w, _MLP_QAT_CLIP)
            down_w = _ste_quant_bank(down_w, _MLP_QAT_CLIP)
...
def quantize_int6_gptq(weight, hessian=None, clip_range=31, block_size=128):
    ...
    W[:, dead[perm]] = 0
    H = H[perm][:, perm]
    # Try increasing damp factors until Cholesky succeeds (wider models need more damping
    # when calibration sequences are few and some input channels are near-zero)
    Hinv = None
    for damp_scale in [0.01, 0.05, 0.10, 0.20]:
        H_try = H.clone()
        damp = damp_scale * torch.mean(torch.diag(H_try)).clamp_min(1e-6)
        H_try[torch.arange(cols), torch.arange(cols)] += damp
        try:
            ...
            continue
    if Hinv is None:
        # Final fallback: plain percentile quantization without GPTQ error compensation
        return _quantize_int6_percentile(t32, clip_range)
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
                        ...
                        mlp_clip_range: int = 31):
    """Quantize model with per-category clip ranges.
    [Insight C] mlp_clip_range: 31=int6(default), 15=int5, 7=int4 for MLP weights."""
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            # [Insight C] Use different clip range for MLP vs attention
            cr = mlp_clip_range if cat == "mlp" else 31
            bit_label = {31: "int6", 15: "int5", 7: "int4"}.get(cr, f"clip{cr}")
...
def main() -> None:
    global _MLP_QAT_ACTIVE, _MLP_QAT_CLIP
    ...
    while True:
        ...
        # [Insight C] Activate MLP bank QAT — uses MLP_QAT_THRESHOLD if set, else falls back to late_qat_threshold
        _mlp_qat_thresh = args.mlp_qat_threshold if args.mlp_qat_threshold > 0 else args.late_qat_threshold
        if args.mlp_qat_enabled and scale < _mlp_qat_thresh and not _MLP_QAT_ACTIVE:
            _MLP_QAT_ACTIVE = True
            _MLP_QAT_CLIP = args.mlp_qat_clip
            log0(f"mlp_bank_qat:enabled step:{step} scale:{scale:.4f} clip:{args.mlp_qat_clip}")
        zero_grad_all()
        ...
        for micro_step in range(grad_accum_steps):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
            # [Insight B] Entropy regularization on MLP banks during warmdown
            if args.entropy_reg_weight > 0 and scale < args.late_qat_threshold:
                for bank in [base_model.mlp_up_bank, base_model.mlp_down_bank]:
                    loss = loss + args.entropy_reg_weight * bank.float().abs().mean()
    ...
    # [Insight A] Autoregressive self-generated calibration — configurable sequence count
    ...
    # [Insight C] Pass configurable MLP clip range for mixed-precision quantization
    log0(f"gptq:quantizing with mlp_clip_range={args.gptq_mlp_clip} (31=int6, 15=int5, 7=int4)")
    quant_result, quant_meta = mixed_quantize_int6(
        unbanked_sd, {"mlp", "attn"}, hessians=hessians, mlp_clip_range=args.gptq_mlp_clip,
    )
    ...
    # Raise dynamo cache limit to avoid FailOnRecompileLimitHit during roundtrip eval
    # (RoPE cache is cold on fresh eval_model; first few calls trigger guard recompiles)
